...
<body> 
<form action="AHP1C4.php" method="POST"> 
...
Apellido: <select type="text" name="apellido"> 
            <?php  
              $cod_ve = array(); 
              $cod_vn = array(); 
             // $c = 0; 
              $consulta1= "SELECT apellido FROM USUARIO"; 
              $resultado1 = mysql_query($consulta1) or die('La consulta fall&oacute;: ' . mysql_error()); 
               
              while($linea1 = mysql_fetch_array($resultado1)){ 
          
               
               echo " <option apellido=\"".$linea1[0]."\">".$linea1[0]."</option>\n"; 
