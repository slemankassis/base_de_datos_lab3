...
<body> 
<form action="AHP1C7.php" method="POST"> 

Seleccione la especialidad del operario: 
	<select type="text" name="especialidad"> 
            <?php  
              $cod_ve = array(); 
              $cod_vn = array(); 
             // $c = 0; 
              $consulta= "SELECT especialidad FROM OPERARIO"; 
                 $resultado = mysql_query($consulta) or die('La consulta fall&oacute;: ' . mysql_error()); 
               
              while($linea = mysql_fetch_array($resultado)){ 
          
               
               echo " <option especialidad=\"".$linea[0]."\">".$linea[0]."</option>\n"; 
