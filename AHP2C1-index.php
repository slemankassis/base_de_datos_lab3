...
<body> 
<form action="AHP2C1.php" method="POST"> 

Seleccione el nombre de ciudad: <select type="text" name="ciudad"> 
            <?php  
              $cod_ve = array(); 
              $cod_vn = array(); 
             // $c = 0; 
              $consulta= "SELECT ciudad FROM DIRECCION"; 
                 $resultado = mysql_query($consulta) or die('La consulta fall&oacute;: ' . mysql_error()); 
               
              while($linea = mysql_fetch_array($resultado)){ 
          
               
               echo " <option ciudad=\"".$linea[0]."\">".$linea[0]."</option>\n"; 
