...
<body> 
<form action="AHP1C6.php" method="POST"> 


Seleccione el estado del presupuesto:
	 <select type="text" name="estado"> 
            <?php  
              $cod_ve = array(); 
              $cod_vn = array(); 
             // $c = 0; 
              $consulta= "SELECT estado FROM PRESUPUESTO"; 
                 $resultado = mysql_query($consulta) or die('La consulta fall&oacute;: ' . mysql_error()); 
               
              while($linea = mysql_fetch_array($resultado)){ 
          
               
               echo " <option estado=\"".$linea[0]."\">".$linea[0]."</option>\n"; 
