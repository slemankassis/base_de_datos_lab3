...
<body> 
<form action="AHP2C5.php" method="POST"> 


ELije un id de incidente: 
	<select type="text" name="id_incidente"> 
            <?php  
              $cod_ve = array(); 
              $cod_vn = array(); 
             // $c = 0; 
              $consulta= "SELECT id_incidente FROM INCIDENTE"; 
                 $resultado = mysql_query($consulta) or die('La consulta fall&oacute;: ' . mysql_error()); 
               
              while($linea = mysql_fetch_array($resultado)){ 
          
               
               echo " <option id_incidente=\"".$linea[0]."\">".$linea[0]."</option>\n"; 
